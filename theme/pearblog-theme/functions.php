...
function pearblog_enqueue_assets() {
    ...
    // V2 Pro Neon (enqueue for V2 Pro landing pages or when v2-pro body class is present)
    if (is_page_template('page-landing-v2-pro.php') || get_option('pearblog_homepage_version') === 'v2-pro') {
        wp_enqueue_style('pearblog-v2-pro-neon', PEARBLOG_URI . '/assets/css/v2-pro-neon.css', array('pearblog-base'), PEARBLOG_VERSION);
        wp_enqueue_script('pearblog-v2-pro-mobile', PEARBLOG_URI . '/assets/js/v2-pro-mobile.js', array('pearblog-app'), PEARBLOG_VERSION, true);
    }
...
/**
 * V2 Pro AJAX Handlers
 */

/**
 * Handle AI analysis request
 */
function v2pro_ajax_ai_analyze() {
    // Verify nonce
    check_ajax_referer('v2pro_ai_nonce', 'nonce');

    $problem = isset($_POST['problem']) ? sanitize_textarea_field($_POST['problem']) : '';

    if (empty($problem) || strlen($problem) < 10) {
        wp_send_json_error(array(
            'message' => 'Proszę opisać problem bardziej szczegółowo.'
        ));
    }

    // Simple keyword-based routing (can be enhanced with AI later)
    $response = v2pro_analyze_problem($problem);
...
/**
 * Analyze problem and return recommendations
 *
 * @param string $problem User's problem description
 * @return array Response with recommendation
 */
function v2pro_analyze_problem($problem) {
    $problem_lower = mb_strtolower($problem);

    // Category detection
    $categories = array(
        'prawo' => array('prawnik', 'sąd', 'pozew', 'rozwód', 'spadek', 'umowa', 'prawo', 'adwokat'),
        'finanse' => array('kredyt', 'pożyczka', 'inwestycja', 'oszczędności', 'bank', 'finanse', 'podatek'),
        'budownictwo' => array('remont', 'budowa', 'dom', 'mieszkanie', 'hydraulik', 'elektryk', 'adaptacja'),
        'zdrowie' => array('lekarz', 'choroba', 'ból', 'diagnoza', 'zdrowie', 'terapia', 'rehabilitacja'),
        'technologia' => array('komputer', 'internet', 'telefon', 'aplikacja', 'software', 'it', 'programowanie'),
    );

    $detected_category = 'ogólne';
    $confidence = 0;

    foreach ($categories as $category => $keywords) {
        $matches = 0;
        foreach ($keywords as $keyword) {
            if (strpos($problem_lower, $keyword) !== false) {
                $matches++;
            }
        }
    ...
    $responses = array(
        'prawo' => array(
            'response' => 'Twoje pytanie dotyczy kwestii prawnych. Polecamy skonsultowanie się ze specjalistą w dziedzinie prawa.',
            'url' => home_url('/eksperci?kategoria=prawo'),
        ),
        'finanse' => array(
            'response' => 'Zagadnienie finansowe wymaga indywidualnej analizy. Skontaktuj się z doradcą finansowym.',
            'url' => home_url('/eksperci?kategoria=finanse'),
        ),
        'budownictwo' => array(
            'response' => 'W sprawach budowlanych najlepiej skonsultować się z fachowcem. Znajdź specjalistę w swojej okolicy.',
            'url' => home_url('/eksperci?kategoria=budownictwo'),
        ),
        'zdrowie' => array(
            'response' => 'Sprawy zdrowotne wymagają profesjonalnej konsultacji medycznej. Znajdź odpowiedniego specjalistę.',
            'url' => home_url('/eksperci?kategoria=zdrowie'),
        ),
        'technologia' => array(
            'response' => 'Problem techniczny? Nasi eksperci IT pomogą Ci znaleźć rozwiązanie.',
            'url' => home_url('/eksperci?kategoria=technologia'),
        ),
        'ogólne' => array(
            'response' => 'Przeanalizowaliśmy Twój problem. Zobacz naszych ekspertów, którzy mogą Ci pomóc.',
            'url' => home_url('/eksperci'),
        ),
    );

    return $responses[$detected_category];
}

/**
 * Track event for analytics
 */
function v2pro_ajax_track_event() {
    ...
    // For now, just log to option for simplicity
    ...
    update_option('pearblog_v2pro_events', $events);

    wp_send_json_success();
}
add_action('wp_ajax_pearblog_track_event', 'v2pro_ajax_track_event');
add_action('wp_ajax_nopriv_pearblog_track_event', 'v2pro_ajax_track_event');

/**
 * Track CTA clicks
 */
function v2pro_ajax_track_cta_click() {
    check_ajax_referer('pearblog_nonce', 'nonce');
    ...
    $key = $cta_id . '_' . $location;

    if (!isset($cta_clicks[$key])) {
        $cta_clicks[$key] = 0;
    }

    $cta_clicks[$key]++;

    update_option('pearblog_v2pro_cta_clicks', $cta_clicks);

    wp_send_json_success();
}
add_action('wp_ajax_pearblog_track_cta_click', 'v2pro_ajax_track_cta_click');
add_action('wp_ajax_nopriv_pearblog_track_cta_click', 'v2pro_ajax_track_cta_click');

/**
 * Track performance metrics
 */
function v2pro_ajax_track_performance() {
    check_ajax_referer('pearblog_nonce', 'nonce');
    ...
    update_option('pearblog_v2pro_performance', $metrics);

    wp_send_json_success();
}
add_action('wp_ajax_pearblog_track_performance', 'v2pro_ajax_track_performance');
add_action('wp_ajax_nopriv_pearblog_track_performance', 'v2pro_ajax_track_performance');
