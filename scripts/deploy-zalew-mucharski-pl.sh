#!/bin/bash
#
# PearBlog Engine - Automated Deployment Script for www.zalew.mucharski.pl
...
WP_PATH="/var/www/www.zalew.mucharski.pl"
DB_NAME="zalew_mucharski_pl"
DB_USER="zalew_user"
...
EOF
    ...
    echo "$DB_PASS" > /root/.zalew_db_pass
    chmod 600 /root/.zalew_db_pass
    print_success "Database password saved to /root/.zalew_db_pass"
}
...
step_7_configure_wordpress() {
    ...
    DB_PASS=$(cat /root/.zalew_db_pass)
...
step_11_add_initial_topics() {
    ...
    TOPICS=(
        "Wędkarstwo na Zalewie Mucharskim - przewodnik dla początkujących"
        "Najlepsze łowiska na Zalewie Mucharskim"
        "Kajaking na Zalewie Mucharskim - trasy i porady"
        "Sporty wodne na Zalewie Mucharskim - co można robić"
        "Windsurfing na Zalewie Mucharskim - warunki i szkoły"
        "Karp i leszcz na Zalewie Mucharskim - techniki połowów"
        "Noclegi nad Zalewem Mucharskim - najlepsze kwatery"
        "Wakeboarding na Zalewie Mucharskim - gdzie szukać szkół"
        "Atrakcje turystyczne wokół Zalewu Mucharskiego"
        "Zasady wędkarskie obowiązujące na Zalewie Mucharskim"
        "Plaże i kąpieliska nad Zalewem Mucharskim"
        "Łódki i sprzęt pływający do wynajęcia na Zalewie"
        "Ryby w Zalewie Mucharskim - jakie gatunki złowisz"
        "Restauracje i bary nad Zalewem Mucharskim"
        "Żeglarstwo na Zalewie Mucharskim - jak zacząć"
        "Sprzęt wędkarski do wędkowania na Zalewie"
        "Festiwale i imprezy nad Zalewem Mucharskim"
        "Camping nad Zalewem Mucharskim - gdzie rozbić namiot"
        "SUP (stand up paddle) na Zalewie Mucharskim"
        "Wycieczki rowerowe wokół Zalewu Mucharskiego"
        "Zalew Mucharski zimą - co robić poza sezonem"
        "Fotografia na Zalewie Mucharskim - najlepsze miejsca"
        "Połów szczupaka na Zalewie Mucharskim"
        "Bezpieczeństwo na wodzie - zasady na Zalewie Mucharskim"
        "Wędkarstwo nocne na Zalewie Mucharskim"
        "Przynęty i zanęty skuteczne na Zalewie Mucharskim"
        "Rodzinny wypoczynek nad Zalewem Mucharskim"
        "Historia Zalewu Mucharskiego - jak powstał zbiornik"
        "Kitesurfing na Zalewie Mucharskim - możliwości"
        "Dojazd i infrastruktura turystyczna Zalewu Mucharskiego"
    )
...
step_12_setup_ssl() {
    ...
    if check_command apache2; then
        certbot --apache -d www.zalew.mucharski.pl --email "$SSL_EMAIL" --agree-tos --non-interactive --redirect
    elif check_command nginx; then
        certbot --nginx -d www.zalew.mucharski.pl --email "$SSL_EMAIL" --agree-tos --non-interactive --redirect
    else
...
main() {
    ...
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║                                                           ║"
    echo "║  PearBlog Engine Deployment for www.zalew.mucharski.pl   ║"
    echo "║                     Version 6.0.0                         ║"
    echo "║       Wędkarstwo, Sporty Wodne, Zalew Mucharski          ║"
    echo "║                                                           ║"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo ""
