...
EOF
    ...
    echo "$DB_PASS" > /root/.poradnik_db_pass
    chmod 600 /root/.poradnik_db_pass
    print_success "Database password saved to /root/.poradnik_db_pass"
}
...
step_7_configure_wordpress() {
    ...
    DB_PASS=$(cat /root/.poradnik_db_pass)
    ...
    # Change table prefix
    wp config set table_prefix "prd_" --allow-root
...
step_11_add_initial_topics() {
    ...
    TOPICS=(
        "Jak skutecznie oszczędzać energię w domu"
        "10 sposobów na lepszy sen"
        "Jak zaplanować domowy budżet"
        "Najlepsze aplikacje do nauki języków"
        "Jak zorganizować małą przestrzeń"
        "Poradnik wyboru laptopa 2026"
        "Jak dbać o rośliny domowe"
        "Szybkie porady kulinarne dla zapracowanych"
