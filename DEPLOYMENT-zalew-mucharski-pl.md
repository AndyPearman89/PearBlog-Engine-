# 🚀 Deployment Guide: www.zalew.mucharski.pl
...
**⚠️ Note:** Before running the script, update `SERVER_IP` in `deploy-zalew-mucharski-pl.sh` with your actual server IP address.
...
dig www.zalew.mucharski.pl +short
# Should return: YOUR_SERVER_IP

nslookup www.zalew.mucharski.pl
```
...
mkdir -p /var/www/www.zalew.mucharski.pl
chown www-data:www-data /var/www/www.zalew.mucharski.pl
```
...
### Download WordPress
```bash
cd /var/www/www.zalew.mucharski.pl
...
CREATE DATABASE zalew_mucharski_pl CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'zalew_user'@'localhost' IDENTIFIED BY 'STRONG_PASSWORD_HERE';
GRANT ALL PRIVILEGES ON zalew_mucharski_pl.* TO 'zalew_user'@'localhost';
FLUSH PRIVILEGES;
EXIT;
```
...
wp config set table_prefix "zlw_" --allow-root
```

### Install WordPress
```bash
wp core install \
  --url="http://www.zalew.mucharski.pl" \
  --title="Zalew Mucharski - Wędkarstwo, Sporty Wodne, Wypoczynek" \
  --admin_user="admin" \
...
mkdir -p /var/www/www.zalew.mucharski.pl/wp-content/mu-plugins

# Copy PearBlog Engine:
cp -r mu-plugins/pearblog-engine /var/www/www.zalew.mucharski.pl/wp-content/mu-plugins/

# Set correct permissions:
chown -R www-data:www-data /var/www/www.zalew.mucharski.pl/wp-content/mu-plugins/pearblog-engine
```

### Deploy Theme
```bash
# Copy theme:
cp -r theme/pearblog-theme /var/www/www.zalew.mucharski.pl/wp-content/themes/
...
# Set permissions:
chown -R www-data:www-data /var/www/www.zalew.mucharski.pl/wp-content/themes/pearblog-theme
```
...
wp option update pearblog_industry "sporty wodne i wędkarstwo na Zalewie Mucharskim" --allow-root
wp option update pearblog_tone "praktyczny, lokalny, dla miłośników wędkarstwa i sportów wodnych na Zalewie Mucharskim" --allow-root
...
```bash
# Add Zalew Mucharski fishing and water sports topics:
wp pearblog queue add "Wędkarstwo na Zalewie Mucharskim - przewodnik dla początkujących" --allow-root
wp pearblog queue add "Najlepsze łowiska na Zalewie Mucharskim" --allow-root
wp pearblog queue add "Kajaking na Zalewie Mucharskim - trasy i porady" --allow-root
wp pearblog queue add "Sporty wodne na Zalewie Mucharskim - co można robić" --allow-root
wp pearblog queue add "Windsurfing na Zalewie Mucharskim - warunki i szkoły" --allow-root
wp pearblog queue add "Karp i leszcz na Zalewie Mucharskim - techniki połowów" --allow-root
wp pearblog queue add "Noclegi nad Zalewem Mucharskim - najlepsze kwatery" --allow-root
wp pearblog queue add "Wakeboarding na Zalewie Mucharskim - gdzie szukać szkół" --allow-root
wp pearblog queue add "Atrakcje turystyczne wokół Zalewu Mucharskiego" --allow-root
wp pearblog queue add "Zasady wędkarskie obowiązujące na Zalewie Mucharskim" --allow-root
...
**Name:** ZALEW_MUCHARSKI_SSH_HOST
**Value:** YOUR_SERVER_IP

**Name:** ZALEW_MUCHARSKI_SSH_USER
**Value:** root

**Name:** ZALEW_MUCHARSKI_SSH_PRIVATE_KEY
**Value:** [Your SSH private key content]

**Name:** ZALEW_MUCHARSKI_WP_PATH
**Value:** /var/www/www.zalew.mucharski.pl

**Name:** ZALEW_MUCHARSKI_ROOT_PASSWORD
...
```bash
# For Apache:
certbot --apache -d www.zalew.mucharski.pl \
...
# For Nginx:
certbot --nginx -d www.zalew.mucharski.pl \
  --email your-email@example.com \
  --agree-tos \
  --non-interactive \
  --redirect
...
```bash
cd /var/www/www.zalew.mucharski.pl
wp option update home "https://www.zalew.mucharski.pl" --allow-root
wp option update siteurl "https://www.zalew.mucharski.pl" --allow-root
```
...
# Visit: https://www.zalew.mucharski.pl/wp-admin
# Login with admin credentials

# Check PearBlog Engine dashboard:
# Visit: https://www.zalew.mucharski.pl/wp-admin/admin.php?page=pearblog-engine
```
...
tail -f /var/www/www.zalew.mucharski.pl/wp-content/pearblog-engine.log

# WordPress debug log:
tail -f /var/www/www.zalew.mucharski.pl/wp-content/debug.log
...
ls -la /var/www/www.zalew.mucharski.pl/wp-content/
```
...
mysql -u zalew_user -p zalew_mucharski_pl
...
**Version:** 6.0.0
**Industry:** Water sports and fishing – Zalew Mucharski specifically
